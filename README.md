...
| **Tests** | xUnit tests. Engine tests under `Tests/Orrery`, `Tests/Pipeline`, `Tests/Mechanism`; small-ISA and hand-crafted-buffer RV64 tests under `Tests/Isa` (`Tests/Isa/RiscV64`); RV32 tests under `Tests/RiscV32/{Isa,Extensions,Pipelines,MultiHart,System,CoSim,Analysis}`; RV64 ISA-conformance tests (real compiled `rv64*-p-*` ELFs, all three pipeline trains) under `Tests/RiscV64/Isa`; cheap-tier RV64 `Tests/RiscV64/{System,MultiHart,CoSim}` (CLINT/HTIF/UART/raw-binary-workload, multi-hart pipeline/atomics/TSO fence, Spike co-sim golden-path + full riscv-tests conformance loop) — RV64 OpenSBI/Linux boot and torture co-sim remain RV32-only, see TODO.md. |

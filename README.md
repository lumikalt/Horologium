...
**ISA coverage:** I/M/A/F (standard), C (compressed 16-bit instructions), V (vector, VLEN=128, V1.0 subset), and UVE (Unlimited Vector Extension, 1D subset). The V subset covers `vsetvli`/`vsetivli`/`vsetvl`, unit-stride loads/stores (`VLE8/16/32`, `VSE8/16/32`, `VLM`, `VSM`), integer ALU (`vadd`, `vsub`, `vand`, `vor`, `vxor`, `vsll`, `vsrl`, `vsra`) in VV/VX/VI variants, and mask comparisons (`vmseq`, `vmsne`, `vmsltu`, `vmslt`, `vmsgtu`, `vmsgt`) with `vm`-bit masking. The UVE 1D subset (encoded in RISC-V custom-0/custom-1 opcode space) covers stream setup (`ss.ld.w`, `ss.st.w`), scalar broadcast (`so.v.dp.w`), element-wise FP arithmetic (`so.a.mul.fp`, `so.a.add.fp`, `so.a.sub.fp`, `so.a.mac.fp`), and stream-loop branch (`so.b.nc`); sufficient for SAXPY-class kernels. Multi-dimensional streams (`ss.sta.*`, `ss.app`, `ss.end`) and the `so.b.ndc.*` branch family are not yet implemented.

...
Scalar cryptography (RV32 forms; RISC-V Cryptography Extensions Volume I: Scalar & Entropy Source Instructions,
v1.0.1): Zknd (NIST AES decryption: `aes32dsi`/`aes32dsmi`), Zkne (NIST AES encryption: `aes32esi`/`aes32esmi`), Zknh
(NIST SHA2: `sha256sig0`/`sha256sig1`/`sha256sum0`/`sha256sum1`, plus the RV32 split-register SHA2-512 forms
`sha512sig0h`/`sha512sig0l`/`sha512sig1h`/`sha512sig1l`/`sha512sum0r`/`sha512sum1r`), Zksed (ShangMi SM4:
`sm4ed`/`sm4ks`), Zksh (ShangMi SM3: `sm3p0`/`sm3p1`), and Zkr (the `seed` entropy-source CSR at 0x015, modeled as a
virtual entropy source per §4.2.3 — every poll succeeds with fresh deterministic pseudorandomness, default M-mode-only
access; RV64-only AES/SHA2-512 forms are a follow-up, see TODO.md).

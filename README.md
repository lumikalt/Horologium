...
| **RiscV32** | RV32IMAFDCV implementation of the Mechanism contract. Includes Zba/Zbb/Zbc/Zbs/Zfh/Zicond/Zawrs/Zicbom/Zicboz/Zimop/Zcmop/Zicntr/Zabha/Zacas and UVE. Zacas adds `amocas.w`; combined with Zabha it also adds narrow `amocas.b`/`amocas.h` compare-and-swap. |
| **RiscV64** | RV64IMAFDAC implementation extending RiscV32 via inheritance. Adds W-suffix ops (ADDW/SUBW/…/ADDIW/…), LD/LWU/SD, LR.D/SC.D/AMO*.D doubleword atomics, `amocas.d` (native single-register 64-bit compare-and-swap — RV32 lacks this form since it needs a register pair to hold a 64-bit value, which the current single-destination-register `ITooth` model doesn't support), C.LD/C.SD/C.ADDIW/C.LDSP/C.SDSP compressed quadrant reassignments, RV64-only Zba (ADD.UW/SH1-3ADD.UW/SLLI.UW) and 64-bit-width overrides of the inherited Zbb/Zbs immediate and register-form ops (6-bit shift-amount mask included), and corrects shift/comparison/LW semantics for 64-bit. Includes an ELF64 loader and an Sv39 page-table walker (satp held in a dedicated 64-bit-wide RV64 CSR rather than RiscV32's 32-bit `CsrFile`, to hold the Sv39 MODE field). The inherited V extension and UVE work unmodified at XLEN=64 (register file and CSRs are width-agnostic); the one XLEN-sensitive path, vlse/vsse strided load-store, reads its stride through an overridable hook so RV64 uses the full 64-bit signed register value instead of RV32's 32-bit sign-extension. |
...
| **Tests** | xUnit tests. Engine tests under `Tests/Orrery`, `Tests/Pipeline`, `Tests/Mechanism`; small-ISA and hand-crafted-buffer RV64 tests under `Tests/Isa` (`Tests/Isa/RiscV64`); RV32 tests under `Tests/RiscV32/{Isa,Extensions,Pipelines,MultiHart,System,CoSim,Analysis}`; RV64 ISA-conformance tests (real compiled `rv64*-p-*` ELFs, all three pipeline trains) under `Tests/RiscV64/Isa`. |

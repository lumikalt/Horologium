...
| **RiscV64** | RV64IMAFDAC implementation extending RiscV32 via inheritance. Adds W-suffix ops (ADDW/SUBW/…/ADDIW/…), LD/LWU/SD, LR.D/SC.D/AMO*.D doubleword atomics, C.LD/C.SD/C.ADDIW/C.LDSP/C.SDSP compressed quadrant reassignments, RV64-only Zba (ADD.UW/SH1-3ADD.UW/SLLI.UW) and 64-bit-width overrides of the inherited Zbb/Zbs immediate ops, and corrects shift/comparison/LW semantics for 64-bit. Includes an ELF64 loader and an Sv39 page-table walker (satp held in a dedicated 64-bit-wide RV64 CSR rather than RiscV32's 32-bit `CsrFile`, to hold the Sv39 MODE field). |

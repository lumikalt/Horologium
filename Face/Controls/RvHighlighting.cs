...
public static class RvHighlighting {
    ...
    public static IHighlightingDefinition GetDefinition(bool isDark) {
        if (isDark) return _dark ??= Load(MakeXshd("#6A9955", "#C586C0", "#569CD6", "#9CDCFE", "#DCDCAA", "#B5CEA8"));
        return _light   ??= Load(MakeXshd("#5A7238", "#7B3F9A", "#0055CC", "#006B99", "#795E26", "#098658"));
    }
    ...
    private static string MakeXshd(
        string comment, string directive, string opcode,
        string register, string label, string number) => $"""
        <?xml version="1.0"?>

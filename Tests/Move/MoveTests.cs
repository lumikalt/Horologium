...
public class MoveTests {
    ...
    public void ImmToReg_AllRegisters() {
        (var t, var s, var m) = Make();
        for (int i = 0; i < 8; i++)
            I(m, i * 4, Mov((byte)i, SRC_IMM, (ushort)(10 + i)));
        t.Run(8);
        for (int i = 0; i < 8; i++)
            Assert.Equal(10 + i, s.R[i]);
    }
    ...
    public void AluCopy_RegisterToRegister() {
        ...
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_COPY));
        I(m, 4,  Mov(R1,      SRC_IMM, 99));
        I(m, 8,  Mov(ALU_IN2, R1));
        I(m, 12, Mov(R2,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluAdd_TwoRegisters() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_ADD));
        I(m, 4,  Mov(R0,      SRC_IMM, 30));
        I(m, 8,  Mov(R1,      SRC_IMM, 12));
        I(m, 12, Mov(ALU_IN1, R0));
        I(m, 16, Mov(ALU_IN2, R1));
        I(m, 20, Mov(R2,      SRC_ALUOUT));
        t.Run(6);
    ...
    public void AluSub_TwoRegisters() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_SUB));
        I(m, 4,  Mov(R0,      SRC_IMM, 50));
        I(m, 8,  Mov(R1,      SRC_IMM, 8));
        I(m, 12, Mov(ALU_IN1, R0));
        I(m, 16, Mov(ALU_IN2, R1));
        I(m, 20, Mov(R2,      SRC_ALUOUT));
        t.Run(6);
    ...
    public void AluAnd() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_AND));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0xFF0F));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 0x0FF0));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluOr() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_OR));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0xF000));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 0x000F));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluXor() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_XOR));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0xFF00));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 0xF0F0));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluNot() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_NOT));
        I(m, 4,  Mov(ALU_IN2, SRC_IMM, 0xABCD));
        I(m, 8,  Mov(R0,      SRC_ALUOUT));
        t.Run(3);
    ...
    public void AluShl() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_SHL));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 1));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 3));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluShr_Logical() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_SHR));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0x8000));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 1));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluSra_Arithmetic() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_SRA));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0x8000));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 1));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluEq_Equal() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_EQ));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 7));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 7));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluLt_SignedLess() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_LT));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 0xFFFF));  // -1 signed
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 1));
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluUlt_UnsignedLess() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_ULT));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 1));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 0xFFFF));  // 65535 unsigned
        I(m, 12, Mov(R0,      SRC_ALUOUT));
        t.Run(4);
    ...
    public void AluNeg() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_NEG));
        I(m, 4,  Mov(ALU_IN2, SRC_IMM, 5));
        I(m, 8,  Mov(R0,      SRC_ALUOUT));
        t.Run(3);
    ...
    public void AluInc() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_INC));
        I(m, 4,  Mov(ALU_IN2, SRC_IMM, 41));
        I(m, 8,  Mov(R0,      SRC_ALUOUT));
        t.Run(3);
        Assert.Equal(42, s.R[0]);
    }
    ...
    public void AluDec() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_DEC));
        I(m, 4,  Mov(ALU_IN2, SRC_IMM, 43));
        I(m, 8,  Mov(R0,      SRC_ALUOUT));
        t.Run(3);
        Assert.Equal(42, s.R[0]);
    }
    ...
    public void AluOp_Persistent_AcrossInstructions() {
        ...
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_ADD));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 10));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 5));    // trigger: alu.out = 15
        I(m, 12, Mov(ALU_IN1, SRC_ALUOUT));    // in1 = 15 (no new alu.op)
        I(m, 16, Mov(ALU_IN2, SRC_IMM, 3));    // trigger: alu.out = 15 + 3 = 18
        I(m, 20, Mov(R0,      SRC_ALUOUT));
        t.Run(6);
    ...
    public void MemLoad_ImmediateAddress() {
        (var t, var s, var m) = Make();
        D(m, DataBase, 0xBEEF);
        I(m, 0, Mov(MEM_LOAD, SRC_IMM, (ushort)DataBase));
        I(m, 4, Mov(R0, SRC_MEMOUT));
        t.Run(2);
    ...
    public void MemLoad_RegisterAddress() {
        (var t, var s, var m) = Make();
        D(m, DataBase + 4, 0x1234);
        I(m, 0, Mov(R1,      SRC_IMM,    (ushort)(DataBase + 4)));
        I(m, 4, Mov(MEM_LOAD, R1));
        I(m, 8, Mov(R0, SRC_MEMOUT));
        t.Run(3);
    ...
    public void MemOut_Persistent_AfterLoad() {
        ...
        D(m, DataBase, 99);
        I(m, 0,  Mov(MEM_LOAD, SRC_IMM, (ushort)DataBase));
        I(m, 4,  Mov(ALU_OP,   SRC_IMM, ALU_COPY));     // intervening instruction
        I(m, 8,  Mov(R0, SRC_MEMOUT));                   // still valid
        t.Run(3);
    ...
    public void MemStore_ImmediateData() {
        (var t, var s, var m) = Make();
        I(m, 0, Mov(MEM_ADDR,  SRC_IMM, (ushort)DataBase));
        I(m, 4, Mov(MEM_STORE, SRC_IMM, 0xCAFE));
        t.Run(2);
        Assert.Equal(0xCAFE, ReadD(m, DataBase));
    }

    [Fact]
    public void MemStore_RegisterData() {
        (var t, var s, var m) = Make();
        I(m, 0, Mov(R5,        SRC_IMM,  0x5555));
        I(m, 4, Mov(MEM_ADDR,  SRC_IMM,  (ushort)DataBase));
        I(m, 8, Mov(MEM_STORE, R5));
        t.Run(3);
        Assert.Equal(0x5555, ReadD(m, DataBase));
    }
    ...
    public void MemStoreAndLoad_RoundTrip() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(MEM_ADDR,  SRC_IMM, (ushort)DataBase));
        I(m, 4,  Mov(MEM_STORE, SRC_IMM, 0xABCD));
        I(m, 8,  Mov(MEM_LOAD,  SRC_IMM, (ushort)DataBase));
        I(m, 12, Mov(R0, SRC_MEMOUT));
        t.Run(4);
    ...
    public void Branch_Taken_NonzeroCondition() {
        ...
        I(m, 0,  Mov(BR_COND,   SRC_IMM, 1));
        I(m, 4,  Mov(BR_TARGET, SRC_IMM, 12));   // taken: PC → 12
        I(m, 8,  Mov(R0,        SRC_IMM, 0));    // skipped
        I(m, 12, Mov(R0,        SRC_IMM, 99));   // executed
    ...
    public void Branch_NotTaken_ZeroCondition() {
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(BR_COND,   SRC_IMM, 0));
        I(m, 4,  Mov(BR_TARGET, SRC_IMM, 12));   // not taken
        I(m, 8,  Mov(R0,        SRC_IMM, 77));   // executed (falls through)
        I(m, 12, Mov(R0,        SRC_IMM, 99));   // also executed (next tick)
    ...
    public void BrCond_Persistent_CanReuse() {
        ...
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(BR_COND,   SRC_IMM, 1));    // set cond = 1
        I(m, 4,  Mov(R0,        SRC_IMM, 5));
        I(m, 8,  Mov(BR_TARGET, SRC_IMM, 16));   // taken
        I(m, 12, Mov(R0,        SRC_IMM, 0));    // skipped
        I(m, 16, Mov(BR_TARGET, SRC_IMM, 24));   // taken again (cond still 1)
        I(m, 20, Mov(R0,        SRC_IMM, 0));    // skipped
        I(m, 24, Mov(R1,        SRC_IMM, 99));   // reached
    ...
    public void Halt_StopsExecution() {
        (var t, var s, var m) = Make();
        I(m, 0, Mov(R0,  SRC_IMM, 42));
        I(m, 4, Mov(HALT, SRC_IMM, 0));
        I(m, 8, Mov(R0,  SRC_IMM, 99));   // must not execute
    ...
    public void SrcPc_ReadsCurrentPc() {
        ...
        I(m, 0, Mov(R0, SRC_IMM, 0));   // placeholder
        I(m, 4, Mov(R0, SRC_IMM, 0));   // placeholder
        I(m, 8, Mov(R0, SRC_PC));       // r0 = PC = 8
        t.Run(3);
        Assert.Equal(8, s.R[0]);
    }
    ...
    public void CountdownLoop_FiveIterations() {
        ...
        (var t, var s, var m) = Make();
        I(m, 0,  Mov(R0,        SRC_IMM, 5));
        I(m, 4,  Mov(ALU_OP,    SRC_IMM, ALU_DEC));
        I(m, 8,  Mov(ALU_IN2,   R0));
        I(m, 12, Mov(R0,        SRC_ALUOUT));
        I(m, 16, Mov(BR_COND,   R0));
        I(m, 20, Mov(BR_TARGET, SRC_IMM, 8));
        I(m, 24, Mov(HALT,      SRC_IMM, 0));
        t.Run(1000);
        Assert.Equal(0, s.R[0]);
    }
    ...
    public void AluIn2_UsesTransportedValue() {
        ...
        I(m, 0,  Mov(ALU_OP,  SRC_IMM, ALU_ADD));
        I(m, 4,  Mov(ALU_IN1, SRC_IMM, 10));
        I(m, 8,  Mov(ALU_IN2, SRC_IMM, 5));   // alu.out = 15
        I(m, 12, Mov(ALU_IN1, SRC_IMM, 20));
        I(m, 16, Mov(ALU_IN2, SRC_IMM, 2));   // alu.out = 22
        I(m, 20, Mov(R0,      SRC_ALUOUT));
        t.Run(6);
    ...
    public void LoadThenCompute() {
        ...
        (var t, var s, var m) = Make();
        D(m, DataBase, 100);
        I(m, 0,  Mov(MEM_LOAD,  SRC_IMM, (ushort)DataBase));  // mem.out = 100
        I(m, 4,  Mov(ALU_OP,    SRC_IMM, ALU_ADD));
        I(m, 8,  Mov(ALU_IN1,   SRC_MEMOUT));                  // in1 = 100
        I(m, 12, Mov(ALU_IN2,   SRC_IMM, 42));                 // trigger: alu.out = 142
        I(m, 16, Mov(MEM_ADDR,  SRC_IMM, (ushort)(DataBase + 2)));
        I(m, 20, Mov(MEM_STORE, SRC_ALUOUT));                  // mem[base+2] = 142
        t.Run(6);
        Assert.Equal(142, ReadD(m, DataBase + 2));
    }
}

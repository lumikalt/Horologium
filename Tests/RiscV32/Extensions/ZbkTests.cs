...
public class ZbkTests {
    ...
    private static uint Pack(int rd, int rs1, int rs2) => RType(0x04, rs2, rs1, 4, rd);
    private static uint Packh(int rd, int rs1, int rs2) => RType(0x04, rs2, rs1, 7, rd);
    private static uint Brev8(int rd, int rs1) => ITypeAlu(0x687, rs1, 5, rd);
    private static uint Zip(int rd, int rs1) => ITypeAlu(0x08F, rs1, 1, rd);
    private static uint Unzip(int rd, int rs1) => ITypeAlu(0x08F, rs1, 5, rd);
    private static uint Xperm4(int rd, int rs1, int rs2) => RType(0x14, rs2, rs1, 2, rd);
    private static uint Xperm8(int rd, int rs1, int rs2) => RType(0x14, rs2, rs1, 4, rd);
    ...
    private static uint Rev8(int rd, int rs1) => ITypeAlu((0x34 << 5) | 0x18, rs1, 5, rd);
    ...
    public void Pack_MatchesIndependentReference(uint a, uint b) {
        ...
        ExecuteResult r = Exec(Pack(3, 1, 2), MakeState((1, a), (2, b)));
    ...
    public void Packh_MatchesIndependentReference(uint a, uint b) {
        ...
        ExecuteResult r = Exec(Packh(3, 1, 2), MakeState((1, a), (2, b)));
        Assert.Equal(expected, r.RegisterResult.Value);
    }
    ...
    public void Pack_RegisterOrder_Rs1IsLowHalfRs2IsHighHalf() {
        ...
        ExecuteResult r = Exec(Pack(3, 1, 2), MakeState((1, 0x0000AAAAu), (2, 0x0000BBBBu)));
    ...
    public void Brev8_IsSelfInverse(uint x) {
        ExecuteResult once = Exec(Brev8(3, 1), MakeState((1, x)));
        ExecuteResult twice = Exec(Brev8(4, 3), MakeState((3, (uint)once.RegisterResult.Value)));
    ...
    public void Brev8_MatchesHandVector(uint input, uint expected) {
        ExecuteResult r = Exec(Brev8(3, 1), MakeState((1, input)));
        Assert.Equal(expected, r.RegisterResult.Value);
    }
    ...
    public void Brev8_DoesNotReorderBytes() {
        ...
        ExecuteResult r = Exec(Brev8(3, 1), MakeState((1, 0x000000F0u)));
    ...
    public void Rev8_StillWorksAfterBrev8SharesFunct7() {
        ExecuteResult r = Exec(Rev8(3, 1), MakeState((1, 0x12345678u)));
    ...
    public void Unzip_UndoesZip(uint x) {
        ExecuteResult zipped = Exec(Zip(3, 1), MakeState((1, x)));
        ExecuteResult unzipped = Exec(Unzip(4, 3), MakeState((3, (uint)zipped.RegisterResult.Value)));
    ...
    public void Zip_UndoesUnzip(uint x) {
        ExecuteResult unzipped = Exec(Unzip(3, 1), MakeState((1, x)));
        ExecuteResult zipped = Exec(Zip(4, 3), MakeState((3, (uint)unzipped.RegisterResult.Value)));
        Assert.Equal(x, zipped.RegisterResult.Value);
    }

    [Fact]
    public void Zip_LowHalfGoesToEvenBits() {
        ExecuteResult r = Exec(Zip(3, 1), MakeState((1, 0x0000FFFFu)));
    ...
    public void Zip_HighHalfGoesToOddBits() {
        ExecuteResult r = Exec(Zip(3, 1), MakeState((1, 0xFFFF0000u)));
    ...
    public void Unzip_EvenBitsGoToLowHalf() {
        ExecuteResult r = Exec(Unzip(3, 1), MakeState((1, 0x55555555u)));
        Assert.Equal(0x0000FFFFu, r.RegisterResult.Value);
    }

    [Fact]
    public void Unzip_OddBitsGoToHighHalf() {
        ExecuteResult r = Exec(Unzip(3, 1), MakeState((1, 0xAAAAAAAAu)));
    ...
    public void Xperm8_MatchesIndependentReference(uint lut, uint idxVec) {
        ExecuteResult r = Exec(Xperm8(3, 1, 2), MakeState((1, lut), (2, idxVec)));
        Assert.Equal(Xperm8Ref(lut, idxVec), r.RegisterResult.Value);
    }

    [Theory]
    [MemberData(nameof(Pairs))]
    public void Xperm4_MatchesIndependentReference(uint lut, uint idxVec) {
        ExecuteResult r = Exec(Xperm4(3, 1, 2), MakeState((1, lut), (2, idxVec)));
        Assert.Equal(Xperm4Ref(lut, idxVec), r.RegisterResult.Value);
    }
    ...
    public void Xperm8_IdentityPermutation() {
        ...
        ExecuteResult r = Exec(
            Xperm8(3, 1, 2), MakeState((1, 0xDDCCBBAAu), (2, 0x03020100u))
        );
    ...
    public void Xperm8_OutOfRangeIndex_YieldsZero() {
        ...
        ExecuteResult r = Exec(
            Xperm8(3, 1, 2), MakeState((1, 0xFFFFFFFFu), (2, 0x00000004u))
        );
    ...
    public void Xperm4_OutOfRangeIndex_YieldsZero() {
        ...
        ExecuteResult r = Exec(
            Xperm4(3, 1, 2), MakeState((1, 0xFFFFFFFFu), (2, 0x00000008u))
        );

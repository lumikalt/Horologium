...
public class ZksTests {
    ...
    private static uint Sm4Ks(int rd, int rs1, int rs2, int bs) => RType((bs << 5) | 0x1A, rs2, rs1, 0, rd);

    // funct7=0x08, shamt selects sub-op: sm3p0=8, sm3p1=9
    private static uint Sm3P0(int rd, int rs1) => ITypeAlu((0x08 << 5) | 8, rs1, 1, rd);
    private static uint Sm3P1(int rd, int rs1) => ITypeAlu((0x08 << 5) | 9, rs1, 1, rd);
    ...
    private static uint Sm3P0Ref(uint x) => x ^ Rol32(x, 9) ^ Rol32(x, 17);
    private static uint Sm3P1Ref(uint x) => x ^ Rol32(x, 15) ^ Rol32(x, 23);

    [Theory]
    [MemberData(nameof(Uint32Values))]
    public void Sm3P0_MatchesReference(uint x) {
        ExecuteResult r = Exec(Sm3P0(3, 1), MakeState((1, x)));
        Assert.Equal(Sm3P0Ref(x), (uint)r.RegisterResult.Value);
    }

    [Theory]
    [MemberData(nameof(Uint32Values))]
    public void Sm3P1_MatchesReference(uint x) {
        ExecuteResult r = Exec(Sm3P1(3, 1), MakeState((1, x)));
        Assert.Equal(Sm3P1Ref(x), (uint)r.RegisterResult.Value);
    }
    ...
    public void Sm4Ed_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            ...
            ExecuteResult r = Exec(Sm4Ed(3, 1, 2, bs), s);
            Assert.Equal(Sm4Ref(rs1, rs2, bs, false), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Sm4Ks_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            Rv32ArchState s = MakeState((1, rs1), (2, rs2));
            ExecuteResult r = Exec(Sm4Ks(3, 1, 2, bs), s);
            Assert.Equal(Sm4Ref(rs1, rs2, bs, true), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Sm4Ed_Bs0_CoversFullSbox() {
        for (var b = 0; b < 256; b++) {
            ...
            ExecuteResult r = Exec(Sm4Ed(3, 1, 2, 0), s);
            Assert.Equal(Sm4Ref(0, (uint)b, 0, false), (uint)r.RegisterResult.Value);
        }
    }
}
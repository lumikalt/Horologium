...
public class ZknTests {
    ...
    private static uint Aes32Dsi(int rd, int rs1, int rs2, int bs) => ZknTests.RType((bs << 5) | 0x15, rs2, rs1, 0, rd);
    private static uint Aes32Dsmi(int rd, int rs1, int rs2, int bs) => ZknTests.RType((bs << 5) | 0x17, rs2, rs1, 0, rd);
    private static uint Aes32Esi(int rd, int rs1, int rs2, int bs) => ZknTests.RType((bs << 5) | 0x11, rs2, rs1, 0, rd);
    private static uint Aes32Esmi(int rd, int rs1, int rs2, int bs) => ZknTests.RType((bs << 5) | 0x13, rs2, rs1, 0, rd);

    // funct7=0x08, shamt selects sub-op: sum0=0, sum1=1, sig0=2, sig1=3
    private static uint Sha256Sum0(int rd, int rs1) => ZknTests.ITypeAlu((0x08 << 5) | 0, rs1, 1, rd);
    private static uint Sha256Sum1(int rd, int rs1) => ZknTests.ITypeAlu((0x08 << 5) | 1, rs1, 1, rd);
    private static uint Sha256Sig0(int rd, int rs1) => ZknTests.ITypeAlu((0x08 << 5) | 2, rs1, 1, rd);
    private static uint Sha256Sig1(int rd, int rs1) => ZknTests.ITypeAlu((0x08 << 5) | 3, rs1, 1, rd);

    private static uint Sha512Sig0H(int rd, int rs1, int rs2) => ZknTests.RType(0x2E, rs2, rs1, 0, rd);
    private static uint Sha512Sig0L(int rd, int rs1, int rs2) => ZknTests.RType(0x2A, rs2, rs1, 0, rd);
    private static uint Sha512Sig1H(int rd, int rs1, int rs2) => ZknTests.RType(0x2F, rs2, rs1, 0, rd);
    private static uint Sha512Sig1L(int rd, int rs1, int rs2) => ZknTests.RType(0x2B, rs2, rs1, 0, rd);
    private static uint Sha512Sum0R(int rd, int rs1, int rs2) => ZknTests.RType(0x28, rs2, rs1, 0, rd);
    private static uint Sha512Sum1R(int rd, int rs1, int rs2) => ZknTests.RType(0x29, rs2, rs1, 0, rd);
    ...
    private static byte GfInverse(byte a) {
        ...
        for (var candidate = 1; candidate < 256; candidate++)
            if (ZknTests.GfMulFull(a, (byte)candidate) == 1)
    ...
    private static byte AesSboxFwdRef(byte x) {
        byte inv = ZknTests.GfInverse(x);
        return (byte)(inv ^ ZknTests.Rotl8(inv, 1) ^ ZknTests.Rotl8(inv, 2) ^ ZknTests.Rotl8(inv, 3) ^
                      ZknTests.Rotl8(inv, 4) ^ 0x63);
    }

    // The inverse S-box is the functional inverse of the forward S-box (both are bijections).
    private static readonly byte[] AesSboxInvRefTable = ZknTests.BuildAesSboxInvRef();

    private static byte[] BuildAesSboxInvRef() {
        var table = new byte[256];
        for (var x = 0; x < 256; x++) table[ZknTests.AesSboxFwdRef((byte)x)] = (byte)x;
        return table;
    }
    ...
    private static uint AesMixColumnByteFwdRef(byte so) {
        byte b0 = ZknTests.GfMulSmall(so, 2);
        byte b3 = ZknTests.GfMulSmall(so, 3);
        return ((uint)b3 << 24) | ((uint)so << 16) | ((uint)so << 8) | b0;
    }

    private static uint AesMixColumnByteInvRef(byte so) {
        byte b0 = ZknTests.GfMulSmall(so, 0xE);
        byte b1 = ZknTests.GfMulSmall(so, 0x9);
        byte b2 = ZknTests.GfMulSmall(so, 0xD);
        byte b3 = ZknTests.GfMulSmall(so, 0xB);
    ...
    private static uint Aes32Ref(uint rs1, uint rs2, int bs, bool inverse, bool mixColumns) {
        ...
        byte so = inverse ? ZknTests.AesSboxInvRef(si) : ZknTests.AesSboxFwdRef(si);
        uint pre = mixColumns
            ? inverse ? ZknTests.AesMixColumnByteInvRef(so) : ZknTests.AesMixColumnByteFwdRef(so)
            : so;
    ...
    public void AesSboxFwdRef_MatchesFips197Landmarks() {
        Assert.Equal(0x63, ZknTests.AesSboxFwdRef(0x00));
        Assert.Equal(0x7C, ZknTests.AesSboxFwdRef(0x01));
        Assert.Equal(0x77, ZknTests.AesSboxFwdRef(0x02));
        Assert.Equal(0x76, ZknTests.AesSboxFwdRef(0x0F));
    }

    [Fact]
    public void AesSboxInvRef_IsFunctionalInverseOfFwd() {
        for (var x = 0; x < 256; x++) Assert.Equal((byte)x, ZknTests.AesSboxInvRef(ZknTests.AesSboxFwdRef((byte)x)));
    }
    ...
    public void Aes32Dsi_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            Rv32ArchState s = ZknTests.MakeState((1, rs1), (2, rs2));
            ExecuteResult r = Exec(ZknTests.Aes32Dsi(3, 1, 2, bs), s);
            Assert.Equal(ZknTests.Aes32Ref(rs1, rs2, bs, true, false), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Aes32Dsmi_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            Rv32ArchState s = ZknTests.MakeState((1, rs1), (2, rs2));
            ExecuteResult r = Exec(ZknTests.Aes32Dsmi(3, 1, 2, bs), s);
            Assert.Equal(ZknTests.Aes32Ref(rs1, rs2, bs, true, true), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Aes32Esi_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            Rv32ArchState s = ZknTests.MakeState((1, rs1), (2, rs2));
            ExecuteResult r = Exec(ZknTests.Aes32Esi(3, 1, 2, bs), s);
            Assert.Equal(ZknTests.Aes32Ref(rs1, rs2, bs, false, false), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Aes32Esmi_MatchesIndependentReference(uint rs1, uint rs2) {
        for (var bs = 0; bs < 4; bs++) {
            Rv32ArchState s = ZknTests.MakeState((1, rs1), (2, rs2));
            ExecuteResult r = Exec(ZknTests.Aes32Esmi(3, 1, 2, bs), s);
            Assert.Equal(ZknTests.Aes32Ref(rs1, rs2, bs, false, true), (uint)r.RegisterResult.Value);
        }
    }
    ...
    public void Aes32Esi_Bs0_CoversFullSbox() {
        for (var b = 0; b < 256; b++) {
            Rv32ArchState s = ZknTests.MakeState((1, 0u), (2, (uint)b));
            ExecuteResult r = Exec(ZknTests.Aes32Esi(3, 1, 2, 0), s);
            Assert.Equal((ulong)ZknTests.AesSboxFwdRef((byte)b), r.RegisterResult.Value);
        }
    }
    ...
    public void Aes32Dsi_Bs0_CoversFullSbox() {
        for (var b = 0; b < 256; b++) {
            Rv32ArchState s = ZknTests.MakeState((1, 0u), (2, (uint)b));
            ExecuteResult r = Exec(ZknTests.Aes32Dsi(3, 1, 2, 0), s);
            Assert.Equal((ulong)ZknTests.AesSboxInvRef((byte)b), r.RegisterResult.Value);
        }
    }
    ...
    private static uint Sha256Sig0Ref(uint x) => ZknTests.Ror32(x, 7) ^ ZknTests.Ror32(x, 18) ^ (x >> 3);
    private static uint Sha256Sig1Ref(uint x) => ZknTests.Ror32(x, 17) ^ ZknTests.Ror32(x, 19) ^ (x >> 10);
    private static uint Sha256Sum0Ref(uint x) => ZknTests.Ror32(x, 2) ^ ZknTests.Ror32(x, 13) ^ ZknTests.Ror32(x, 22);
    private static uint Sha256Sum1Ref(uint x) => ZknTests.Ror32(x, 6) ^ ZknTests.Ror32(x, 11) ^ ZknTests.Ror32(x, 25);
    ...
    public void Sha256Sig0_MatchesReference(uint x) {
        ExecuteResult r = Exec(ZknTests.Sha256Sig0(3, 1), ZknTests.MakeState((1, x)));
        Assert.Equal(ZknTests.Sha256Sig0Ref(x), (uint)r.RegisterResult.Value);
    }

    [Theory]
    [MemberData(nameof(Uint32Values))]
    public void Sha256Sig1_MatchesReference(uint x) {
        ExecuteResult r = Exec(ZknTests.Sha256Sig1(3, 1), ZknTests.MakeState((1, x)));
        Assert.Equal(ZknTests.Sha256Sig1Ref(x), (uint)r.RegisterResult.Value);
    }

    [Theory]
    [MemberData(nameof(Uint32Values))]
    public void Sha256Sum0_MatchesReference(uint x) {
        ExecuteResult r = Exec(ZknTests.Sha256Sum0(3, 1), ZknTests.MakeState((1, x)));
        Assert.Equal(ZknTests.Sha256Sum0Ref(x), (uint)r.RegisterResult.Value);
    }

    [Theory]
    [MemberData(nameof(Uint32Values))]
    public void Sha256Sum1_MatchesReference(uint x) {
        ExecuteResult r = Exec(ZknTests.Sha256Sum1(3, 1), ZknTests.MakeState((1, x)));
        Assert.Equal(ZknTests.Sha256Sum1Ref(x), (uint)r.RegisterResult.Value);
    }
    ...
    private static ulong Sha512Sig0Ref(ulong x) => ZknTests.Ror64(x, 1) ^ ZknTests.Ror64(x, 8) ^ (x >> 7);
    private static ulong Sha512Sig1Ref(ulong x) => ZknTests.Ror64(x, 19) ^ ZknTests.Ror64(x, 61) ^ (x >> 6);

    private static ulong Sha512Sum0Ref(ulong x) =>
        ZknTests.Ror64(x, 28) ^ ZknTests.Ror64(x, 34) ^ ZknTests.Ror64(x, 39);

    private static ulong Sha512Sum1Ref(ulong x) =>
        ZknTests.Ror64(x, 14) ^ ZknTests.Ror64(x, 18) ^ ZknTests.Ror64(x, 41);

    public static TheoryData<ulong> Uint64Values => new() {
    ...
    public void Sha512Sig0_SplitPair_ReconstructsFullSigma0(ulong v) {
        var lo = (uint)v;
        var hi = (uint)(v >> 32);
        uint t0 = (uint)Exec(ZknTests.Sha512Sig0L(5, 10, 11), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        uint t1 = (uint)Exec(ZknTests.Sha512Sig0H(6, 11, 10), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        Assert.Equal(ZknTests.Sha512Sig0Ref(v), ((ulong)t1 << 32) | t0);
    }
    ...
    public void Sha512Sig1_SplitPair_ReconstructsFullSigma1(ulong v) {
        var lo = (uint)v;
        var hi = (uint)(v >> 32);
        uint t0 = (uint)Exec(ZknTests.Sha512Sig1L(5, 10, 11), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        uint t1 = (uint)Exec(ZknTests.Sha512Sig1H(6, 11, 10), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        Assert.Equal(ZknTests.Sha512Sig1Ref(v), ((ulong)t1 << 32) | t0);
    }
    ...
    public void Sha512Sum0_SplitPair_ReconstructsFullSum0(ulong v) {
        var lo = (uint)v;
        var hi = (uint)(v >> 32);
        uint t0 = (uint)Exec(ZknTests.Sha512Sum0R(5, 10, 11), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        uint t1 = (uint)Exec(ZknTests.Sha512Sum0R(6, 11, 10), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        Assert.Equal(ZknTests.Sha512Sum0Ref(v), ((ulong)t1 << 32) | t0);
    }
    ...
    public void Sha512Sum1_SplitPair_ReconstructsFullSum1(ulong v) {
        var lo = (uint)v;
        var hi = (uint)(v >> 32);
        uint t0 = (uint)Exec(ZknTests.Sha512Sum1R(5, 10, 11), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        uint t1 = (uint)Exec(ZknTests.Sha512Sum1R(6, 11, 10), ZknTests.MakeState((10, lo), (11, hi)))
            .RegisterResult.Value;
        Assert.Equal(ZknTests.Sha512Sum1Ref(v), ((ulong)t1 << 32) | t0);
    }
}

...
/// <summary>
///     <see cref="DaeTrain" />'s <see cref="ExecuteResult.RequestBlock" /> support. Unlike the ROB-based
///     (<see cref="OooTrain" />) or checkpoint-based (<see cref="CprTrain" />) fixes, this one needs no
///     rollback at all: `ecall` is barrier-class (<c>IsBarrierClass</c>), so <c>ExecuteBarrier</c> only
///     ever runs once both the Access and Execute lanes are fully drained — nothing younger is ever in
///     flight when a barrier executes, and its own undo-log entries were just cleared immediately
///     before. A still-blocked barrier is therefore a pure retry-in-place: redirect fetch back to its
///     own Pc and let it be re-decoded and re-dispatched as a fresh instruction. See
///     <c>DaeCore.ExecuteBarrier</c>'s <c>RequestBlock</c> branch.
/// </summary>
public class DaeRequestBlockGuardTests {
    ...
    public void BlockingEcall_RetriesInPlace_ThenProceedsNormallyOnceCleared() {
        ...
        BitConverter.TryWriteBytes(bytes.AsSpan(0), DaeRequestBlockGuardTests.Ecall);
        BitConverter.TryWriteBytes(bytes.AsSpan(4), DaeRequestBlockGuardTests.AddiX1Plus1);
        BitConverter.TryWriteBytes(bytes.AsSpan(8), DaeRequestBlockGuardTests.Ebreak);
        mem.Load(0x00, bytes);

        var handler = new BlockThenClearHandler(3);
        var mechanism = new Rv32Mechanism(syscallHandler: handler);
        var train = new DaeTrain(mechanism, mem);

        train.Run(1000);

        Assert.True(train.IsIdle);
        // Executed (ExecuteBarrier → handler.Handle) four times — three blocked attempts plus
        // the one that finally cleared.
    ...
    public void BlockedEcall_ResumesWhenAnotherHartClearsTheSharedFutexWord() {
        ...
        BitConverter.TryWriteBytes(hart0Bytes.AsSpan(0), DaeRequestBlockGuardTests.Ecall);
        BitConverter.TryWriteBytes(hart0Bytes.AsSpan(4), Addi(2, 2, 1));
        BitConverter.TryWriteBytes(hart0Bytes.AsSpan(8), DaeRequestBlockGuardTests.Ebreak);
        ...
        BitConverter.TryWriteBytes(hart1Bytes.AsSpan(8), DaeRequestBlockGuardTests.Ebreak);
        ...
        var train0 = new DaeTrain(mech0, mem);
        var train1 = new DaeTrain(mech1, mem, 0x40);

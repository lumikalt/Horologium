...
public class BenchmarkTests(ITestOutputHelper output) {
    ...
    private static (FlatMemory Mem, ulong EntryPoint, ulong TohostAddr) Load(string name) {
        var wl = new ElfWorkload(ElfPath(name), BenchmarkTests.MemoryBytes);
        var mem = new FlatMemory(BenchmarkTests.MemoryBytes);
        wl.Load(mem);
        return (mem, wl.EntryPoint, wl.FindSymbol("tohost"));
    }
    ...
    public void SingleCycle_Passes(string name) {
        (FlatMemory mem, ulong entry, ulong tohost) = Load(name);
        var train = new SingleCycleTrain(new RvMechanism(), mem, entry);
    ...
    public void OoOE_Passes(string name) {
        (FlatMemory mem, ulong entry, ulong tohost) = Load(name);
        var train = new OooeTrain(new RvMechanism(), mem, entry);
        train.Run(20_000_000);
        AssertPass(ReadTohostLow(mem, tohost), name);
    }
    ...
    public void BenchmarkPerfSummary() {
        ...
        foreach (object[] row in AllBenchmarks()) {
            ...
            (FlatMemory mem, ulong entry, ulong _) = Load(name);
            var train = new OooeTrain(new RvMechanism(), mem, entry);

...
public class MoesifCacheTests {
    ...
    private static (MoesifBus bus, FlatMemory backing) MakeBus(int backingSize = 0x1000) {
        var backing = new FlatMemory(backingSize);
        var bus = new MoesifBus(backing);
        return (bus, backing);
    }

    private static MoesifCache MakeCache(MoesifBus bus) => new(
        bus, MoesifCacheTests.Capacity, MoesifCacheTests.Ways, MoesifCacheTests.Block
    );
    ...
    public void ReadMiss_InstallsLine_Exclusive() {
        ...
        backing.Load(0x00, new byte[MoesifCacheTests.Block]);
        MoesifCache cache = MakeCache(bus);

        _ = cache.Read(0x00, 4);

        Assert.Equal(MoesifState.Exclusive, cache.StateOf(0x00));
    ...
    public void ReadHit_ReturnsData_ExclusiveUnchanged() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        var payload = new byte[MoesifCacheTests.Block];
        payload[0] = 0xAB;
        payload[1] = 0xCD;
        payload[2] = 0xEF;
        payload[3] = 0x12;
    ...
    public void WriteHit_Exclusive_TransitionsToModified_NoWriteback() {
        ...
        Assert.Equal(MoesifState.Modified, cache.StateOf(0x00));
    ...
    public void Evict_Modified_WritesBackToBacking() {
        ...
        (MoesifBus bus, FlatMemory backing) = MakeBus(0x400);
        backing.Load(0x000, new byte[MoesifCacheTests.Block]);
        backing.Load(0x080, new byte[MoesifCacheTests.Block]);
        backing.Load(0x100, new byte[MoesifCacheTests.Block]);
        MoesifCache cache = MakeCache(bus);
    ...
    [Fact]
    public void TwoCaches_BothRead_SupplierDowngrades_ReaderGetsForward() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        backing.Load(0x00, new byte[MoesifCacheTests.Block]);
        MoesifCache c0 = MakeCache(bus);
        MoesifCache c1 = MakeCache(bus);

        _ = c0.Read(0x00, 4); // c0: E
        _ = c1.Read(0x00, 4); // BusRead: c0 E→S supplies; c1 takes the Forward role
    ...
    public void TwoCaches_FullCoherenceRoundTrip() {
        ...
        Assert.Equal(MoesifState.Shared, c0.StateOf(0x00));
        Assert.Equal(MoesifState.Owned, c1.StateOf(0x00));
    }
    ...
    public void SilentExclusiveUpgrade_CancelsReservation() {
        ...
        var bus = new MoesifBus(backing, table);
        MakeCache(bus);
        MoesifCache cache1 = MakeCache(bus); // hart B's cache
        ...
        Assert.Equal(MoesifState.Exclusive, cache1.StateOf(0x200));

        // Hart B writes 0x200 → E→M silent upgrade.  Must cancel hart A's reservation.
        cache1.Write(0x200, 0xABCD, 4);

        Assert.Equal(MoesifState.Modified, cache1.StateOf(0x200));
    ...
    public void SharedUpgrade_CancelsReservation() {
        ...
        var table = new ReservationTable();
        var bus = new MoesifBus(backing, table);
        MoesifCache cache0 = MakeCache(bus);
        MoesifCache cache1 = MakeCache(bus);
    ...
    public void Load_InvalidatesAllCaches() {
        ...
        var fresh = new byte[MoesifCacheTests.Block];
        fresh[0] = 0xFF;
        c0.Load(0x00, fresh);
    ...
    public void CleanLine_ModifiedLine_WritesBackAndKeepsExclusive() {
        ...
        Assert.Equal(MoesifState.Exclusive, cache.StateOf(0x00)); // still cached, now clean
    ...
    public void CboFlush_MultiHart_MakesWriteVisibleToRemoteRead() {
        ...
        Assert.Equal(MoesifState.Modified, cache0.StateOf(0x00));

        cache0.FlushLine(0x00); // cbo.flush: writeback + invalidate

        Assert.Equal(MoesifState.Invalid, cache0.StateOf(0x00));
        // cache1 BusRead sees authoritative data from backing
        Assert.Equal(0xF00DUL, cache1.Read(0x00, 4));
        Assert.Equal(MoesifState.Exclusive, cache1.StateOf(0x00));
    }

    // ── MOESIF ownership & cache-to-cache supply ──────────────────────────────
    ...
    public void ExclusiveLine_SuppliedCacheToCache_OnPeerRead() {
        ...
        ulong val = c1.Read(0x00, 1); // c0 E→S, supplies clean block; c1 becomes forwarder
    ...
    public void OwnedLine_SuppliesEveryLaterReader_BackingStaysStale() {
        ...
        Assert.Equal(MoesifState.Owned, c0.StateOf(0x00));
        Assert.Equal(MoesifState.Shared, c1.StateOf(0x00));
        Assert.Equal(MoesifState.Shared, c2.StateOf(0x00));
        Assert.Equal(0, c0.Writebacks);
    ...
    public void OwnedLine_Eviction_WritesBackToBacking() {
        ...
        Assert.Equal(MoesifState.Invalid, c0.StateOf(0x000));
    ...
    public void CleanLine_OwnedLine_WritesBackAndDropsToShared() {
        ...
        Assert.Equal(MoesifState.Shared, c0.StateOf(0x00)); // peers still share — not E
    ...
    public void CrossBoundaryRead_SeesOwnDirtyLine() {
        ...
        Assert.Equal(MoesifState.Modified, cache.StateOf(0x00)); // state untouched
    }
    ...
    public void CrossBoundaryRead_SeesPeerOwnedLine() {
        ...
        Assert.Equal(MoesifState.Owned, c0.StateOf(0x00)); // sync does not change state
    }
    ...
    public void PeerSupply_ChargesPeerSupplyLatency_NotMissLatency() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        backing.Load(0x00, new byte[MoesifCacheTests.Block]);
        var c0 = new MoesifCache(bus, MoesifCacheTests.Capacity, MoesifCacheTests.Ways, MoesifCacheTests.Block, 10, 3);
        var c1 = new MoesifCache(bus, MoesifCacheTests.Capacity, MoesifCacheTests.Ways, MoesifCacheTests.Block, 10, 3);
    ...
    public void WriteMiss_SharedPeersOnly_FillsFromBacking() {
        ...
        // Evict the forwarder so only c0's plain S copy remains (set 0, 2 ways).
        _ = c1.Read(0x080, 4);
        _ = c1.Read(0x100, 4);
        Assert.Equal(MoesifState.Invalid, c1.StateOf(0x00));

        c2.Write(0x00, 0x9999, 4); // write miss: plain S holders don't forward
    ...
    // ── MESIF Forward state: clean-line cache-to-cache supply ────────────────

    [Fact]
    public void Forward_MigratesToNewestReader_MemoryStaysSilent() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        var payload = new byte[MoesifCacheTests.Block];
        payload[0] = 0x42;
        backing.Load(0x00, payload);
        ...
        _ = c0.Read(0x00, 1);         // c0: E
        _ = c1.Read(0x00, 1);         // c0→S supplies; c1: F
        ulong val = c2.Read(0x00, 1); // c1 F→S supplies; c2: F

        Assert.Equal(0x42UL, val);
        Assert.Equal(MoesifState.Shared, c0.StateOf(0x00));
        Assert.Equal(MoesifState.Shared, c1.StateOf(0x00));
        Assert.Equal(MoesifState.Forward, c2.StateOf(0x00));
        Assert.Equal(1, c2.PeerSupplies); // every clean re-read was cache-to-cache
    }

    [Fact]
    public void ForwardHolder_Evicted_NoWriteback_NextReaderBecomesForward() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        backing.Load(0x000, new byte[MoesifCacheTests.Block]);
        MoesifCache c0 = MakeCache(bus);
        MoesifCache c1 = MakeCache(bus);
        ...
        _ = c0.Read(0x000, 4); // c0: E
        _ = c1.Read(0x000, 4); // c0: S, c1: F
        // Evict the forwarder (set 0, 2 ways): F is clean, so no writeback.
        _ = c1.Read(0x080, 4);
        _ = c1.Read(0x100, 4);
        Assert.Equal(MoesifState.Invalid, c1.StateOf(0x000));
        Assert.Equal(0, c1.Writebacks);

        _ = c2.Read(0x000, 4); // no forwarder left → memory supplies, c2 takes F

        Assert.Equal(0, c2.PeerSupplies);
        Assert.Equal(MoesifState.Forward, c2.StateOf(0x000));
        Assert.Equal(MoesifState.Shared, c0.StateOf(0x000));
    }

    [Fact]
    public void ForwardHolder_Write_UpgradesToModified_InvalidatesSharers() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        backing.Load(0x00, new byte[MoesifCacheTests.Block]);
        MoesifCache c0 = MakeCache(bus);
        MoesifCache c1 = MakeCache(bus);
        ...
        _ = c0.Read(0x00, 4);  // c0: E
        _ = c1.Read(0x00, 4);  // c0: S, c1: F
        c1.Write(0x00, 88, 4); // F→M: BusReadInvalidate snoops c0

        Assert.Equal(MoesifState.Invalid, c0.StateOf(0x00));
        Assert.Equal(MoesifState.Modified, c1.StateOf(0x00));
        Assert.Equal(0, c1.Writebacks); // clean upgrade, no writeback anywhere
        Assert.Equal(0, c0.Writebacks);
    }

    [Fact]
    public void ForwardHolder_ForwardsOnWriteMiss() {
        (MoesifBus bus, FlatMemory backing) = MakeBus();
        var payload = new byte[MoesifCacheTests.Block];
        payload[8] = 0x99;
        backing.Load(0x00, payload);
        ...
        _ = c0.Read(0x00, 4);      // c0: E
        _ = c1.Read(0x00, 4);      // c0: S, c1: F
        c2.Write(0x00, 0x1234, 4); // RFO: c1 (F) forwards, c0 (S) just invalidates

        Assert.Equal(MoesifState.Invalid, c0.StateOf(0x00));
        Assert.Equal(MoesifState.Invalid, c1.StateOf(0x00));
        Assert.Equal(MoesifState.Modified, c2.StateOf(0x00));
        Assert.Equal(1, c2.PeerSupplies);
        Assert.Equal(0x99UL, c2.Read(0x08, 1)); // untouched byte came via the forward
    }
}
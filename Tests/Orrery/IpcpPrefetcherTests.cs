...
public sealed class IpcpPrefetcherTests {
    ...
    public void Gs_DenseRegion_TriggersPrefetch() {
        ...
        for (var i = 0; i < 48; i++) Fire(p, (ulong)(0x3000 + i * 4), (ulong)(i * 32), buf);
    ...
    public void Gs_Tentative_WhenEnteringNewRegionAfterDense() {
        ...
        for (var i = 1; i < 48; i++) Fire(p, pc, (ulong)(i * 32), buf);
    ...
    public void Gs_SparseRegion_NoGsPrefetch() {
        ...
        Fire(p, pc, 0, buf); // init
        for (var i = 1; i < 10; i++) Fire(p, pc, (ulong)(i * 32), buf);
    ...
    public void NoCrossPagePrefetch_GsStopsAtBoundary() {
        ...
        for (var i = 1; i < 47; i++) Fire(p, pc, 2048UL + (ulong)(i * 32), buf);
        ...
        Assert.True(cnt > 0);
        for (var i = 0; i < cnt; i++) Assert.True(buf[i] < 4096UL, $"Prefetch {buf[i]} crossed page boundary");
    }
    ...
    public void Constructor_NonPow2BlockBytes_Throws() {
        Assert.Throws<ArgumentException>(() => new IpcpPrefetcher(33));
    }

    [Fact]
    public void Constructor_TooManyLinesPerRegion_Throws() {
        // blockBytes=16, regionBytes=2048 → linesPerRegion=128 > 64 → throws
        Assert.Throws<ArgumentException>(() => new IpcpPrefetcher(16, regionBytes: 2048));
    }
}
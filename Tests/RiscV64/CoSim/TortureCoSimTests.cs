...
/// <summary>
///     RV64 counterpart of <see cref="Tests.RiscV32.CoSim.TortureCoSimTests" /> — runs
///     the pre-generated RV64 torture tests under Spike lock-step co-simulation.
///     <para>
///         Torture tests are random RV64IMAFD instruction sequences produced by
///         <c>TestBinaries/gen_torture.py --xlen 64</c> (seed 42, 20 tests × 300
///         instructions). The compiled ELFs live under <c>TestBinaries/torture64/</c> and
///         are committed alongside their <c>.S</c> sources; rebuild with
///         <c>make torture64</c> in that directory.
///     </para>
///     <para>
...
public class TortureCoSimTests {
    ...
    private static readonly string TortureDir =
        Path.Combine(AppContext.BaseDirectory, "torture64");
    ...
    public static IEnumerable<object[]> TortureElfs() =>
        Directory.EnumerateFiles(TortureCoSimTests.TortureDir, "torture_*.elf")
                 .OrderBy(p => p)
                 .Select(p => new object[] { Path.Combine("torture64", Path.GetFileName(p)), });
    ...
    private static void RunCoSim(
        string elfName,
        Func<Rv64Mechanism, FlatMemory, ulong, ICommitObserver, object> factory
    ) {
        RequireSpikeOrSkip();

        string elfPath = ElfPath(elfName);
        var workload = new Rv64ElfWorkload(elfPath);
    ...
    private static SingleCycleTrain SingleCycle(Rv64Mechanism m, FlatMemory mem, ulong pc, ICommitObserver o) =>

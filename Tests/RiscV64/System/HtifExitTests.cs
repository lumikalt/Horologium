...
/// <summary>
/// RV64 counterpart of <see cref="Tests.RiscV32.System.HtifExitTests"/> — verifies
/// HTIF tohost-exit termination across all three trains <em>without</em> Spike, so
/// the halt paths stay covered in CI where the co-sim harness can't run.
/// <para>
/// htif64.elf does real RV64IM work, then exits via the tohost register (writing
...
public class HtifExitTests {
    private static string HtifElf => Path.Combine(AppContext.BaseDirectory, "htif64.elf");
    ...
    private static (RevolutionResult Result, long Retired, uint TohostLow) Run(string train, bool configureTohost) {
        var workload = new Rv64ElfWorkload(HtifElf, HtifExitTests.MemoryBytes);
        var mem = new FlatMemory(workload.MemorySize, workload.BaseAddress);
        workload.Load(mem);

        ulong tohost = workload.FindSymbol("tohost");
        var mech = new Rv64Mechanism(configureTohost ? tohost : null);

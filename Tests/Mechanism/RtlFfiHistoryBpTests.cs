...
/// <summary>
///     Drives <see cref="RtlFfiHistoryBp" /> against the verilated Chisel
///     L-TAGE, which mirrors <see cref="LTageBp" /> bit-for-bit. The differential
...
/// </summary>
public sealed class RtlFfiHistoryBpTests {
    [SkippableFact]
    public void Loader_DetectsShimAbi() {
        ...
        Assert.IsType<RtlFfiHistoryBp>(tage);
        Assert.IsType<RtlFfiBp>(gshare);
    ...
    public void LoopBranch_LearnedByLoopPredictor() {
        ...
        using var p = new RtlFfiHistoryBp(RtlTageLibrary.Path);
    ...
    public void DifferentialOooSequence_MatchesCSharpLTage() {
        Skip.If(RtlTageLibrary.Path is null, "verilator toolchain unavailable — skipping.");
        using var rtl = new RtlFfiHistoryBp(RtlTageLibrary.Path);
        var reference = new LTageBp();

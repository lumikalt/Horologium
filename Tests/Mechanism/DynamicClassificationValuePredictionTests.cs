...
public class DynamicClassificationValuePredictionTests {
    ...
    public void Unclassified_NoPredictionDuringLearningWindow() {
        ...
        p.Update(pc, default, 10); // 1st of 3 learning values
        Assert.False(p.TryPredict(pc, default, out _));
        p.Update(pc, default, 20); // 2nd of 3
        Assert.False(p.TryPredict(pc, default, out _));
    ...
    public void ConstantDeltaHistory_ClassifiesToComputational_AndEventuallyPredicts() {
        ...
        p.Update(pc, default, 30); // Stride: seed, Init
        p.Update(pc, default, 40); // stride=10 vs 0: mismatch, stays Init, stride now 10
        p.Update(pc, default, 50); // stride=10 vs 10: match, Init -> Transient
        p.Update(pc, default, 60); // stride=10 vs 10: match, Transient -> Steady
    ...
    public void NonConstantDeltaHistory_ClassifiesToContext_AndEventuallyPredicts() {
        ...
        p.Update(pc, default, 5);
        p.Update(pc, default, 99);
        p.Update(pc, default, 3);
        ...
        for (var i = 0; i < 5000; i++) p.Update(pc, default, steady);
    ...
    public void Update_OnlyTrainsTheAssignedComponent() {
        ...
        p.Update(pc, default, 10);
        p.Update(pc, default, 20);
        for (var i = 0; i < 10; i++) p.Update(pc, default, 20 + 10UL * (ulong)(i + 1));
    ...
    public void EvictionFromComputational_ReturnsToUnclassified_AndReclassifiesToContext() {
        ...
        Assert.True(p.TryPredict(pc, default, out ulong reclassified));
    ...
    public void EvictionFromContext_BecomesPermanentlyDontPredict() {
        ...
        for (var i = 0; i < 20; i++) p.Update(pc, default, steady);
        Assert.False(p.TryPredict(pc, default, out _));
    }
}

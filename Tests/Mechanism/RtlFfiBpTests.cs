...
/// <summary>
///     Drives <see cref="RtlFfiBp" /> against the verilated Chisel gshare.
///     The RTL module mirrors <see cref="GshareBp" /> bit-for-bit, so beyond basic
...
/// </summary>
public sealed class RtlFfiBpTests {
    private static RtlFfiBp Open() {
        Skip.If(RtlBpLibrary.Path is null, "verilator toolchain unavailable — skipping.");
        return new RtlFfiBp(RtlBpLibrary.Path);
    }

    [SkippableFact]
    public void ColdMiss_PredictsFallThrough() {
        using RtlFfiBp p = Open();
    ...
    public void DifferentialStream_MatchesCSharpGshare() {
        Skip.If(RtlBpLibrary.Path is null, "verilator toolchain unavailable — skipping.");
        using var rtl = new RtlFfiBp(RtlBpLibrary.Path);
        var reference = new GshareBp(); // historyBits = 8 = the Chisel default

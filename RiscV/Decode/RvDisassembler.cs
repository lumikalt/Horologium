...
public static class RvDisassembler {
    ...
    private static readonly string[] FpAbi = [
    ...
    private static string Xi(int r) => RvDisassembler.IntAbi[r & 31];
    private static string Xf(int r) => RvDisassembler.FpAbi[(r - 32) & 31];
    ...
    public static string Disassemble(object? payload, ulong pc) => payload switch {
        // Pseudo-instructions (order before their base instructions)
        RvAddi { Rd: 0, Rs1: 0, Imm: 0, } => "nop",
        RvAddi { Rs1: 0, } op             => $"li {Xi(op.Rd)}, {op.Imm}",
        RvAddi { Imm: 0, } op             => $"mv {Xi(op.Rd)}, {Xi(op.Rs1)}",
        RvAdd { Rs1: 0, } op              => $"mv {Xi(op.Rd)}, {Xi(op.Rs2)}",
        RvJal { Rd: 0, } op               => $"j {Tgt(pc, op.Imm)}",
        RvJal { Rd: 1, } op               => $"jal {Tgt(pc, op.Imm)}",
        RvJalr { Rd: 0, Rs1: 1, Imm: 0, } => "ret",
        RvJalr { Rd: 0, Imm: 0, } op      => $"jr {Xi(op.Rs1)}",
        RvBeq { Rs2: 0, } op              => $"beqz {Xi(op.Rs1)}, {Tgt(pc, op.Imm)}",
        RvBne { Rs2: 0, } op              => $"bnez {Xi(op.Rs1)}, {Tgt(pc, op.Imm)}",
        RvBlt { Rs2: 0, } op              => $"bltz {Xi(op.Rs1)}, {Tgt(pc, op.Imm)}",
        RvBge { Rs2: 0, } op              => $"bgez {Xi(op.Rs1)}, {Tgt(pc, op.Imm)}",
        RvBlt { Rs1: 0, } op              => $"bgtz {Xi(op.Rs2)}, {Tgt(pc, op.Imm)}",
        RvBge { Rs1: 0, } op              => $"blez {Xi(op.Rs2)}, {Tgt(pc, op.Imm)}",
        RvXori { Imm: -1, } op            => $"not {Xi(op.Rd)}, {Xi(op.Rs1)}",
        RvSub { Rs1: 0, } op              => $"neg {Xi(op.Rd)}, {Xi(op.Rs2)}",
        RvSltiu { Imm: 1, } op            => $"seqz {Xi(op.Rd)}, {Xi(op.Rs1)}",
        RvSltu { Rs1: 0, } op             => $"snez {Xi(op.Rd)}, {Xi(op.Rs2)}",

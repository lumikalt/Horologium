...
///         only place the wrapped <see cref="CbpNgFfiBp" /> is ever touched — it calls the
...
///         <see cref="GshareBp" /> by default) does, via <see cref="Predict" /> and
...
///     </para>
/// </summary>
public sealed class CbpNgCommitDrivenBp : IBranchKindAwareBranchPredictor, IDisposable {
    private readonly IBranchPredictor _fetchPredictor;
    private readonly CbpNgFfiBp _harcom;

    /// <summary>
    ///     Loads the native shim library at <paramref name="libraryPath" /> (see
    ///     <see cref="CbpNgFfiBp" />) and wraps it for reentrant-safe use.
    /// </summary>
    ...
    ///     <see cref="GshareBp" />. Must be safe under speculation (support
    ...
    /// </param>
    public CbpNgCommitDrivenBp(string libraryPath, IBranchPredictor? fetchPredictor = null) {
        _harcom = new CbpNgFfiBp(libraryPath);
        _fetchPredictor = fetchPredictor ?? new GshareBp();
    }

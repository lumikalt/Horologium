...
///     <para>
///         Augments a <see cref="TageScLBp" /> baseline with a small H2P-targeted
...
///         suppressed (§4.6) via <see cref="LTageBp.SuppressTageUpdate" /> to stop
...
///     </para>
/// </summary>
public sealed class BullseyeBp : TageScLBp {
    ...
    private const int LocalTableSize = 1 << BullseyeBp.LocalTableBits;
    ...
    protected override bool ResolvePrediction(ulong pc, int provider, bool tagePred) {
        ...
        bool tageStrong = TageUsefulness(pc, provider) == BullseyeBp.MaxUsefulness ||
                          Math.Abs(TageScore(pc, provider) + ScSum(pc)) > TageScLBp.ScThreshold;
    ...
    protected override void OnAfterUpdate(
        ...
        // (pre-training) state, the same trick TageScLBp.OnAfterUpdate uses for itself.
        int preTotal = preScore + ScSum(pc);
        bool tageScPred = Math.Abs(preTotal) > TageScLBp.ScThreshold ? preTotal >= 0 : provPred;
    ...
    private HitEntry TouchHit(ulong pc) {
        ...
        if (_hit.Count >= BullseyeBp.HitCapacity) EvictLruHit();
    ...
    private void TryAdmit(ulong pc, HitEntry hit) {
        ...
        if (_h2P.Count >= BullseyeBp.H2PCapacity && !EvictWeakestH2P()) return; // no room, nobody evictable
    ...
    private void UpdateH2PEntry(ulong pc, H2PEntry e, bool taken, bool tageScPred) {
        ...
        e.LocalHistory = ((e.LocalHistory << 1) | (taken ? 1UL : 0UL)) &
                         ((1UL << (BullseyeBp.LocalWindowOffsets[^1] + BullseyeBp.LocalWindowWidths[^1]))
                        - 1);
        ...
        if (tageScPred == taken) {
            ...
            e.FilterStreak++;
            if (e.FilterStreak >= BullseyeBp.FilterStreakLimit) e.Filtered = true;
        }

        // §4.2.2: evict once past the warm-up trial window if neither perceptron ever
        // became confident, or if the entry has gone stale.
        if (e.TrialCount >= BullseyeBp.TrialWindow && !localStrong && !globalStrong) _h2P.Remove(pc);
    }
    ...
    private bool EvictWeakestH2P() {
        ...
        foreach ((ulong key, H2PEntry e) in _h2P) {
            if (e.TrialCount < BullseyeBp.TrialWindow) continue; // still in warm-up, protected
            double score = Math.Max(WinRate(e.LocalWins, e.LocalTotal), WinRate(e.GlobalWins, e.GlobalTotal));
            if (_clock - e.LastTouch > BullseyeBp.StaleTimeout) score = -1; // stale entries evict first
    ...
    private static bool WinRateStrong(uint wins, uint total) =>
        total >= BullseyeBp.MinSamplesForWinRate &&
        100 * wins / total >= BullseyeBp.WinRatePercent;
    ...
    private static void TrainPerceptron(ref int theta, ref int tc, bool mispredicted, int magnitude) {
        if (mispredicted) {
            tc++;
            if (tc > BullseyeBp.ThresholdTcBound) {
                theta++;
                tc = 0;
            }
        }
    ...
    private static ulong WindowBits(ulong localHistory, int window) {
        int offset = BullseyeBp.LocalWindowOffsets[window];
        int width = BullseyeBp.LocalWindowWidths[window];
    ...
    private static int Hash(ulong pc, ulong windowBits, int salt) {
        ...
        x ^= x >> 33;
        return (int)(x & (BullseyeBp.LocalTableSize - 1));
    }
    ...
    private int GlobalOutput(H2PEntry e) {
        int fold = FoldGlobal();
        int y = e.GlobalBias;
        for (var i = 0; i < BullseyeBp.GlobalFoldBits; i++)
    ...
    private int FoldGlobal() {
        ulong h = Ghr;
        var res = 0;
        for (var sh = 0; sh < LTageBp.MaxHist; sh += BullseyeBp.GlobalFoldBits)
            res ^= (int)((h >> sh) & ((1 << BullseyeBp.GlobalFoldBits) - 1));
        return res & ((1 << BullseyeBp.GlobalFoldBits) - 1);
    }

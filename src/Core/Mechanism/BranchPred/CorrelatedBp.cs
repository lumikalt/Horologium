...
public sealed class CorrelatedBp : IBranchPredictor {
    ...
    /// <summary>
    ///     Constructs a CorrelatedBp with the given history length.
    /// </summary>
    /// <param name="m">
    ///     Bits in the BHR.
    /// </param>
    ...
    public CorrelatedBp(int m = 2, int n = 2, int bhtSize = 1024) {
...
public sealed class GshareBp : IBranchPredictor {
    private readonly ulong[] _btb;
    private readonly int _ghrMask;
    private readonly SpeculativeGlobalHistory _hist;
    private readonly byte[] _pht;
    ...
    public GshareBp(int historyBits = 8) {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(historyBits);

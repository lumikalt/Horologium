...
///         runs the predictor's table training against that shadow. This is the predict-time history of
///         the committing branch (valid because in-order commit and fetch means any branch
...
///         committed shadow, so the behavior is bit-identical to a plain single-register history.
///     </para>
/// </summary>
internal sealed class SpeculativeGlobalHistory {
    ...
    ///     the working history ends equal to the advanced committed shadow (old behavior).
    /// </summary>

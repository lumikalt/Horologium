...
///         <see cref="TageScLPredictor.ResolvePrediction" />'s existing
///         <c>TageScore(pc,provider) + ScSum(pc)</c>, reusing the same
///         <see cref="TageScLPredictor.ScThreshold" /> magnitude gate — the natural translation of
...
///         <see cref="TageScLPredictor.ScThreshold" /> (a fixed constant) rather than gem5's
///         per-PC dynamically-adapted <c>pUpdateThreshold</c> table — the same simplification this
///         codebase's <see cref="TageScLPredictor" /> already makes for its own SC threshold. None
...
///     </para>
/// </summary>
public sealed class MultiperspectivePerceptronPredictor : TageScLPredictor {
    ...
    private readonly uint[] _blurryPath = new uint[MultiperspectivePerceptronPredictor.BlurryDepth]; // dedup'd pc>>5
    private readonly uint[] _imliCounter = new uint[4]; // only [0] (backward) and [3] (forward) are read
    private readonly bool[] _modHist = new bool[MultiperspectivePerceptronPredictor.ModHistDepth];
    private readonly ushort[] _modPath = new ushort[MultiperspectivePerceptronPredictor.ModHistDepth];
    private readonly ushort[] _recency = new ushort[MultiperspectivePerceptronPredictor.RecencyAssoc]; // MRU stack
    ...
    /// </summary>
    public MultiperspectivePerceptronPredictor() {
        _tables = new short[MultiperspectivePerceptronPredictor.NumFeatures][];
        for (var f = 0; f < MultiperspectivePerceptronPredictor.NumFeatures; f++)
            _tables[f] = new short[MultiperspectivePerceptronPredictor.TableSize];
    }
    ...
    protected override void OnAfterUpdate(
        ...
        if (scPred != taken || Math.Abs(preTotal) < TageScLPredictor.ScThreshold) TrainMpp(pc, taken);
    ...
    private int MppSum(ulong pc) {
        Span<int> idx = stackalloc int[MultiperspectivePerceptronPredictor.NumFeatures];
        ComputeIndices(pc, idx);
        var y = 0;
        for (var f = 0; f < MultiperspectivePerceptronPredictor.NumFeatures; f++)
            y += (int)(MultiperspectivePerceptronPredictor.Coeff[f] * _tables[f][idx[f]]);
        return y;
    }
    ...
    private void TrainMpp(ulong pc, bool taken) {
        ...
        for (var f = 0; f < MultiperspectivePerceptronPredictor.NumFeatures; f++) {
            ...
            if (taken) {
                if (c < MultiperspectivePerceptronPredictor.CounterMax) c++;
            }
            else {
                if (c > MultiperspectivePerceptronPredictor.CounterMin) c--;
            }
        }
    }
    ...
    private void ComputeIndices(ulong pc, Span<int> idx) {
        ...
        idx[MultiperspectivePerceptronPredictor.FBlurryPath] = Index(BlurryHash(), hpc, false);
        idx[MultiperspectivePerceptronPredictor.FRecencyPos] = Index(RecencyHash((ushort)(pc >> 2)), hpc, false);
        idx[MultiperspectivePerceptronPredictor.FGhistModPath] = Index(GhistModPathHash(), hpc, false);
        idx[MultiperspectivePerceptronPredictor.FImli1] = Index(_imliCounter[0], hpc, false);
        // imli_mask1 (0x70) also XORs table 4's index with the backward IMLI counter.
        idx[MultiperspectivePerceptronPredictor.FImli4] = Index(_imliCounter[3], hpc, true);
    }
    ...
    private int Index(ulong hash, uint hpc, bool extraImli0) {
        ...
        return (int)(h % MultiperspectivePerceptronPredictor.TableSize);
    }

    private uint BlurryHash() {
        uint x = 0;
        for (var i = 0; i < MultiperspectivePerceptronPredictor.BlurryDepth; i++) x += _blurryPath[i];
        return x;
    }

    private ulong RecencyHash(ushort pc2) {
        for (var i = 0; i < MultiperspectivePerceptronPredictor.RecencyAssoc; i++)
            if (_recency[i] == pc2)
                return (ulong)(i * MultiperspectivePerceptronPredictor.TableSize
                             / MultiperspectivePerceptronPredictor.RecencyAssoc);
        return MultiperspectivePerceptronPredictor.TableSize - 1; // miss bucket
    }

    private ulong GhistModPathHash() {
        ulong x = 0;
        for (var i = 0; i < MultiperspectivePerceptronPredictor.ModHistDepth; i++)
    ...
    private void AdvanceHistories(ulong pc, bool taken, ulong target) {
        var z = (uint)(pc >> MultiperspectivePerceptronPredictor.BlurryScale);
        if (_blurryPath[0] != z) {
            for (int i = MultiperspectivePerceptronPredictor.BlurryDepth - 1; i > 0; i--)
                _blurryPath[i] = _blurryPath[i - 1];
            _blurryPath[0] = z;
        }
        ...
        var hpc = (uint)(pc ^ (pc >> 2));
        if (hpc % MultiperspectivePerceptronPredictor.ModHistDivisor == 0) {
            for (int i = MultiperspectivePerceptronPredictor.ModHistDepth - 1; i > 0; i--) {
    ...
    private void InsertRecency(ushort pc2) {
        int i;
        for (i = 0; i < MultiperspectivePerceptronPredictor.RecencyAssoc; i++)
            if (_recency[i] == pc2)
                break;
        if (i == MultiperspectivePerceptronPredictor.RecencyAssoc) {
            i = MultiperspectivePerceptronPredictor.RecencyAssoc - 1;
            _recency[i] = pc2;
        }

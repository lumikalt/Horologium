...
///         bits instead of a full subtract, and randomizing a fraction of bits on insert — are
...
///     </para>
/// </summary>
public sealed class HyprePredictor : IBranchPredictor {
    private const int VectorBits = 1024;
    private const int VectorWords = HyprePredictor.VectorBits / 64; // 16, exact — no partial-word masking needed
    ...
    private readonly HdVector _baseNotTaken = new(HyprePredictor.VectorBits);
    private readonly HdVector _baseTaken = new(HyprePredictor.VectorBits);
    ...
    private readonly ulong[] _queryBuf = new ulong[HyprePredictor.VectorWords];
    ...
    /// </param>
    public HyprePredictor(int[]? histLengths = null) {
        _histLengths = histLengths ?? HyprePredictor.DefaultHistLengths;
        ...
        for (var i = 0; i < _histLengths.Length; i++) {
            _taken[i] = new HdVector(HyprePredictor.VectorBits);
            _notTaken[i] = new HdVector(HyprePredictor.VectorBits);
        }

        _localHistory = new byte[HyprePredictor.LocalTableSize];

        // D/2 + 3σ, σ = √D⁄2 (binomial(D, 0.5) standard deviation) — see class doc §2.2.
        var std = (int)Math.Round(Math.Sqrt(HyprePredictor.VectorBits) / 2.0);
        _threshold = HyprePredictor.VectorBits / 2 + 3 * std;
    }
        ...
        _hist.Commit(
            taken, () => {
                ...
                _localHistory[li] = (byte)(((_localHistory[li] << 1) | (taken ? 1 : 0)) &
                                           ((1 << HyprePredictor.LocalHistoryBits) - 1));
            }
        );
    }
    ...
    private static int LocalIdx(ulong pc) => (int)((pc >> 2) & (HyprePredictor.LocalTableSize - 1));
    ...
    private static int MatchCount(ReadOnlySpan<ulong> query, HdVector v) {
        ...
        return HyprePredictor.VectorBits - mismatches;
    }
    ...
    private sealed class HdVector {
        public readonly sbyte[] Counters;
        public readonly ulong[] SignBits;

        public HdVector(int bits) {
            Counters = new sbyte[bits];
            SignBits = new ulong[bits / 64];
        }
    }
}
...
///         most recent direction) and whether a freshly-constructed <see cref="TageScLPredictor" />
...
///         <see cref="TageScLPredictor" /> baseline that this class keeps continuously trained
///         via delegation — the same "override a shared baseline for a known-hard subset"
///         layering <see cref="RunltsPredictor" /> and <see cref="LvcpPredictor" /> use,
...
///     </para>
/// </summary>
public sealed class BranchNetPredictor : IBranchPredictor {
    ...
    public void Update(ulong pc, bool taken, ulong actualTarget) {
        ...
        _committedGhr = ((_committedGhr << 1) | (taken ? 1UL : 0UL)) & BranchNetPredictor.HistoryMask;
    ...
    public void SpeculativeHistoryUpdate(ulong pc, bool predictedTaken) {
        ...
        _speculative = true;
        _ghr = ((_ghr << 1) | (predictedTaken ? 1UL : 0UL)) & BranchNetPredictor.HistoryMask;
    }
    ...
    public void RestoreHistory(in BranchHistoryCheckpoint checkpoint, ulong pc, bool actualTaken) {
        ...
        _speculative = true;
        _ghr = (((checkpoint.Global & BranchNetPredictor.HistoryMask) << 1) | (actualTaken ? 1UL : 0UL))
             & BranchNetPredictor.HistoryMask;
    }
    ...
    public static BranchNetPredictor FromProfile(BranchProfiler profiler) {
        List<ulong> h2p = profiler.Stats
                                  ...
                                  .Take(BranchNetPredictor.MaxH2PBranches)
                                  .Select(kv => kv.Key)
                                  .ToList();
        ...
        return new BranchNetPredictor(models);
    }

    private static bool[] HistoryBits(ulong ghr) {
        var bits = new bool[BranchNetPredictor.HistoryLength];
        for (var i = 0; i < BranchNetPredictor.HistoryLength; i++) bits[i] = ((ghr >> i) & 1) != 0;
        return bits;
    }
    ...
    ///     <see cref="BranchNetPredictor.FromProfile" />.
    /// </summary>
    public sealed class BranchProfiler(IDecoder decoder) : ICommitObserver {
        ...
        public void OnCommit(ulong pc, uint rawEncoding, IArchState state) {
            ...
            if (list.Count < BranchNetPredictor.MaxSamplesPerBranch) list.Add((HistoryBits(_ghr), taken));

            _ghr = ((_ghr << 1) | (taken ? 1UL : 0UL)) & BranchNetPredictor.HistoryMask;
        }
    }
}
...
///     performs the offline backpropagation pass (see <see cref="BranchNetPredictor" />'s

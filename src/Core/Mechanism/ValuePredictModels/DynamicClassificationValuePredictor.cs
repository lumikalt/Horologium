...
///     tech report CMuART-1998-01, §3.2.3 "Efficient Dynamic Scheme (Zero Overlap)"). Unlike
///     <see cref="HybridValuePredictor" /> (which always queries and always trains both components,
///     gating only on agreement), this assigns each static instruction to <em>at most one</em>
///     component — matching the paper's stated goal of avoiding wasted table space in components
...
///         Horologium's <see cref="HybridValuePredictor" /> composition only has two component roles
///         (context-based and computational) rather than three, and <see cref="VtagePredictor" />
...
///         component (typically <see cref="StridePredictor" />), <em>anything else</em> &#8594; the
///         context-based component (typically <see cref="VtagePredictor" />, playing the paper's
...
///         to exactly one component. If a classified PC's own component predicts confidently
...
///         (computational) sends the PC back to Unclassified to relearn against whatever new pattern
///         emerged.
///     </para>
///     <para>
...
///         in flight (see <see cref="StridePredictor" />'s own doc comment for exactly this class of
...
///         evicts on the very first ordinary, ungraceful misprediction after arming — e.g. a
///         Context-classified PC backed by <see cref="VtagePredictor" /> (whose whole premise is
...
///         and is a small, deliberately deferred refinement — see TODO.md — not built here since no
///         test in this codebase measures eviction *rate*, only that eviction *happens* at all;
///         tuning a threshold against that would be tuning blind.
///     </para>
///     <para>
///         Tagless and PC-indexed throughout (classification table, history table, and the "armed"
///         bit), matching this file family's existing convention (see <see cref="LvpPredictor" />):
...
///         desync that component's own internal bookkeeping (e.g. <see cref="StridePredictor" />'s
...
///     </para>
/// </summary>
public sealed class DynamicClassificationValuePredictor : IValuePredictor {
    ...
    /// <summary>
    ///     Classify (paper §3.2.3): examines the 3-value history and assigns exactly one component.
    ...
    ///     (evicted from the context/FCM-role component) or returns to Unclassified to relearn

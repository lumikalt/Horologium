...
public sealed class SppPrefetcher : IPrefetcher {
    ...
    private void PtTrain(int sig, int delta) {
        int idx = SppPrefetcher.PtIndex(sig);
    ...
    private int Predict(int sig, ulong line, ulong basePage, Span<ulong> targets) {
        ...
        var count = 0;
        int curSig = sig;
        long curLine = (long)line;
        var pathConf = 100;

        for (var depth = 0; depth < SppPrefetcher.MaxLookahead; depth++) {
            int idx = SppPrefetcher.PtIndex(curSig);
    ...
    private int StFind(ulong page) {
        ...
        int set = (int)(page & (SppPrefetcher.StEntries / 4 - 1)); // 64 sets × 4 ways
        for (var w = 0; w < 4; w++) {
            int i = set * 4 + w;
    ...
    private void StAllocate(ulong page, int offset, int sig) {
        var tag = (ushort)(page & 0xFFFF);
        int set = (int)(page & (SppPrefetcher.StEntries / 4 - 1));
        int victim = set * 4;
        for (var w = 1; w < 4; w++) {
            int i = set * 4 + w;
            if (!_st[i].Valid) {
    ...
    private bool PfCheckAndInsert(ulong line) {
        int idx = SppPrefetcher.PfIndex(line);
        byte tag = SppPrefetcher.PfTag(line);
    ...
    private void PfDemandCheck(ulong line) {
        int idx = SppPrefetcher.PfIndex(line);
        if (!_pf[idx].Valid || _pf[idx].Tag != SppPrefetcher.PfTag(line) || _pf[idx].Counted) return;

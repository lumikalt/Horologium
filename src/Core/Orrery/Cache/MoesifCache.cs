...
public sealed class MoesifCache : IMemory {
    ...
    public MoesifCache(
        ...
        for (var s = 0; s < sets; s++) {
            ...
            _lruAge[s] = new int[Ways];
            _state[s] = new MoesifState[Ways]; // all Invalid
    ...
    ///     instead of from backing memory. Defaults to <see cref="MissLatency" />.
    /// </summary>
    ...
    /// <summary>
    ///     Read and write misses that were filled cache-to-cache by a peer
    ///     (MOESIF supply on reads, read-for-ownership forwarding on writes).
    /// </summary>
    ...
    public void Write(ulong address, ulong value, int bytes) {
        ...
        if (offset + bytes > BlockBytes) {
            ...
            ulong end = address + (ulong)bytes;
            for (ulong a = LineBase(address); a < end; a += (ulong)BlockBytes) {

# Minimal Linux 6.12 kernel for RV64 NOMMU QEMU virt (Horologium simulation).
#
# RV64 counterpart of linux-rv32.nix. Uses the same nommu_virt_defconfig, but
# WITHOUT the 32-bit.config fragment — nommu_virt_defconfig defaults to RV64
# NOMMU + M-mode, so the kernel links at PAGE_OFFSET = 0x80000000 (RAM base)
# and must be loaded there directly, same as the RV32 build — no OpenSBI.
#
...
#
# Build:   nix build .#linux-rv64
# Output:  result/share/linux/Image   (plain binary)
...
let
  rv64 = pkgsCross.riscv64;
  xcc  = "riscv64-unknown-linux-gnu-";
in
stdenv.mkDerivation rec {
  pname   = "linux-rv64-nommu-virt";
  version = "6.12.94";
  ...
  nativeBuildInputs = [
    rv64.buildPackages.gcc
    rv64.buildPackages.binutils
    flex
    bison
  ...
  configurePhase = ''
    # Base config: nommu_virt_defconfig defaults to RV64 NOMMU (QEMU virt, no MMU,
    # VirtIO, ns16550a UART) — no 32-bit.config fragment needed here.
    ...
    # per reg-shift=0 in virt64.dts) so kernel log appears on UART from the start.
    ./scripts/config \
  ...
  meta = {
    description = "Linux ${version} — RV64 NOMMU QEMU virt for Horologium simulation";

# OpenSBI generic platform for RV64.
...
#
# Note: uses pkgsCross.riscv64 (riscv64-unknown-linux-gnu) rather than the
# bare-metal riscv64-embedded toolchain. The linux-gnu toolchain supports PIE,
# which OpenSBI ≥ 1.2 requires. It still produces valid bare-metal binaries.
#
# Build:   nix build .#opensbi-rv64
...
stdenv.mkDerivation {
  ...
  # python3 for host-side build scripts; rv64 linux-gnu gcc for cross-assembly
  ...
  meta = {
    description = "OpenSBI fw_jump for RV64 generic platform (Horologium virt machine)";
